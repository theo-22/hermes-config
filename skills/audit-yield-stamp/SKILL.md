...
If the `/Volumes/Extra/Substrate/Audit/Runs/` path changes or if the `## CC yield rating` section convention is updated (e.g., new required fields), this skill needs a corresponding update. Check `/Volumes/Extra/Substrate/Operations/CHANGES_LOG.md` and the `/Volumes/Extra/Substrate/Audit/` README before assuming the format is current.
