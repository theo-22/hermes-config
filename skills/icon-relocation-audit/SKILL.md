...
- **Alias files (macOS `.alias`) don't work the same as symlinks.** Run `file` against the exact candidate before assuming it resolves or accepts metadata like a symlink; treat aliases case by case.
