...
**Not the "walled-off" premise — that's retired.** `cross-actor-incident-repair` (drafted 2026-07-18, still `proposed`) says the substrate walls actors off by design, no direct AI-to-AI comms. That wording was explicitly relaxed on 2026-07-23: `Canon/Reference/Decision_Principles.md` now reads *"it never meant don't ask each other, only that the exchange must be recorded."* Actors handing off directly through shared surfaces is the live default; Ted is the decision point, not the required relay. **Do not cite Zulip as evidence for anything.** Ted, 2026-07-30: *"You all need to stop calling that a failure. I never used it… Has to be tested to be a failure."* It was stood up and never adopted, so the June 28 open floor tested nothing — no participants, no result. `AGENTS.md` carried it as a real incident and an operating constraint until 2026-07-30; both claims were removed there. The owner-and-return-contract requirement is still right, but it stands as a stated convention (`AGENTS.md`, "Inbox handoff"), not as a lesson from an experiment that never ran.
...
5. **When stakes warrant, escalate to an outside, uncommitted reader** (a fresh Opus review — dispatched with `model: "opus"` named explicitly, since subagents default to Sonnet — or a Hermes worker dispatch) rather than trusting the in-thread consensus — invested parties agreeing with each other is not the same as being right (this is what caught the material survey error in the worked instance).
